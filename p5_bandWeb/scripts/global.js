...
function resetFields(whichform){
	...
	for(var i=0;i<whichform.length;i++){
		...
		var check=element.placeholder||element.getAttribute("Attribute");
		if(!check) continue;
		element.onfocus=function(){
			var text=this.placeholder||this.getAttribute("Attribute");
			if(this.value==text){
				this.className='';
				this.value="";
			}
...
function prepareForms() {
  for (var i=0; i<document.forms.length; i++) {
    ...
    thisform.onsubmit = function() {
      ...
      if (submitFormWithAjax(this, article)) return false;  //如果ajax成功了，则返回false阻止表单默认提交；如果失败则让表单继续默认提交
      return true;
    }
  }
}
